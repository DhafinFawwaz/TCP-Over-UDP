...
void Server::run() {
    ...
    char sync_buffer[MAXLINE];
    auto sync_buffer_size = connection.recv(sync_buffer, MAXLINE, &addr, &len);
    ((char*)sync_buffer)[sync_buffer_size] = '\0';
    cout << YEL << "[i] [Handshake] [S=2266133599] Received SYN request from " << inet_ntoa(addr.sin_addr) << ":" << addr.sin_port << endl << COLOR_RESET;

    cout << BLU << "[i] [Handshake] [A=3165500899] [S=2266133600] Sending SYN-ACK request to " << inet_ntoa(addr.sin_addr) << ":" << addr.sin_port << COLOR_RESET << endl;
    char sync_ack_buffer[MAXLINE]; ((char*)sync_ack_buffer)[0] = '\0';
    connection.send(inet_ntoa(addr.sin_addr), addr.sin_port, (void*)sync_ack_buffer, sync_buffer_size);

    char ack_buffer[MAXLINE];
    ((char*)ack_buffer)[0] = '\0';
    auto ack_buffer_size = connection.recv(ack_buffer, MAXLINE, &addr, &len);
    cout << YEL << "[i] [Handshake] [A=3165500900] Received ACK request from " << inet_ntoa(addr.sin_addr) << ":" << addr.sin_port << COLOR_RESET << endl;
